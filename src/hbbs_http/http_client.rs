...
mod idle_timeout_tests {
    ...
    //!
    //! The distinction being tested is the whole point of D2. An unreachable host fails at CONNECT,
    //! which the connect timeout has always covered. What was uncovered is a peer that completes the
    //! handshake, answers with headers, and then goes silent forever — previously that sat there until
    //! the 180 s total data ceiling, and a genuinely slow-but-progressing transfer was killed by the
    //! same ceiling for the opposite reason. So the stub below deliberately SUCCEEDS at connecting and
    //! at sending headers, then stops.
    ...
    async fn a_stalled_response_body_is_abandoned_at_the_idle_timeout() {
        ...
        assert!(outcome.is_err(), "a body that never arrives must fail, not hang or return empty");
        ...
        assert!(
            elapsed >= std::time::Duration::from_millis(1500) && elapsed < std::time::Duration::from_secs(20),
    ...
    async fn the_shipped_client_abandons_a_stalled_body() {
        ...
        assert!(outcome.is_err(), "the shipped client must abandon a stalled body");
