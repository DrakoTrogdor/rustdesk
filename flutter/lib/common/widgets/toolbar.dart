...
import 'package:flutter_hbb/sulltec_remote/credential_injection.dart';
...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  // SullTec: inject a stored login credential as keystrokes (console-launched sessions only).
  if (isDefaultConn &&
      !ffiModel.viewOnly &&
      ffiModel.keyboard &&
    ...
    v.add(
      TTextMenu(
          ...
          onPressed: () => injectLoginCredential(ffi, id, sessionId, 'username')),
    );
    v.add(
      TTextMenu(
          child: Text(translate('Inject password')),
          onPressed: () => injectLoginCredential(ffi, id, sessionId, 'password')),
    );
  }
  // restart
  if (isDefaultConn &&
